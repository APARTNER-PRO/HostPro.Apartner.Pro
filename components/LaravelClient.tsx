...
const CSS = `
  ...
  .hp-plan-card.popular{background:linear-gradient(160deg,rgba(244,63,94,.1),rgba(225,29,72,.08));border-color:rgba(244,63,94,.35);box-shadow:0 0 60px rgba(244,63,94,.12)}
  ...
  .hp-plan-btn.primary{background:linear-gradient(135deg,#F43F5E,#E11D48);border:none}
  ...
  .laravel-plans-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; align-items: start; }
  @media (max-width: 900px) { .laravel-plans-grid { grid-template-columns: repeat(2, 1fr); } }
  @media (max-width: 600px) { .laravel-plans-grid { grid-template-columns: 1fr; } }
`

export default function LaravelClient({ lang }: { lang: Lang }) {
  const T = getT(lang)
  ...
  return (
    <>
      ...
      <section className="grid-bg" style={{ position:'relative',minHeight:'90vh',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden', paddingTop: 80 }}>
        <div className="orb hp-orb-pulse" style={{ width:600,height:600,background:'radial-gradient(circle,rgba(244,63,94,.15),transparent)',top:'10%',left:'50%',transform:'translateX(-50%)' }} />
        <div style={{ textAlign:'center',maxWidth:820,padding:'0 24px',position:'relative',zIndex:2 }}>
          <div className="hp-badge" style={{ display:'inline-flex',alignItems:'center',gap:8,background:'rgba(244,63,94,.1)',border:'1px solid rgba(244,63,94,.25)',borderRadius:100,padding:'6px 16px',fontSize:13,color:'#FB7185',marginBottom:32,fontWeight:500 }}>{T.laravel.badge}</div>
          <h1 style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'clamp(40px,7vw,72px)',lineHeight:1.1,letterSpacing:'-2px',marginBottom:24 }}>
            <span style={{ display:'block',color:'#F0F4FF' }}>{T.laravel.title1}</span>
            <span className="grad-text" style={{ display:'block', background: 'linear-gradient(135deg,#F43F5E,#FB923C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{T.laravel.title2}</span>
          </h1>
          <p style={{ fontSize:'clamp(16px,2vw,19px)',color:'rgba(240,244,255,.6)',lineHeight:1.7,maxWidth:640,margin:'0 auto 40px',fontWeight:300 }}>{T.laravel.sub}</p>
          <Link href="#laravel-pricing" className="btn-primary hp-cta-link" style={{ background: 'linear-gradient(135deg,#F43F5E,#FB923C)', boxShadow:'0 0 40px rgba(244,63,94,.25)',fontSize:17,padding:'16px 36px' }}>{T.hero.cta} →</Link>
        </div>
      </section>
      ...
      <section id="laravel-features">
        <div className="section-container">
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:24 }}>
            {T.laravel.features.map((f: any, i: number)=>(
      ...
      <section id="laravel-pricing">
        <div className="section-container">
          <FadeIn style={{ textAlign:'center',marginBottom:48 }}>
            ...
            <p style={{ fontSize:18,color:'rgba(240,244,255,.5)',fontWeight:300 }}>{T.laravel.pricingSub}</p>
          </FadeIn>
          ...
          <FadeIn>
            <div className="laravel-plans-grid">
              {T.pricing.plans.slice(2, 5).map((plan: any, i: number)=>(
                <div key={i} className={`hp-plan-card${i===1?' popular':''}`}>
                  {i===1&&<div style={{ position:'absolute',top:-12,left:'50%',transform:'translateX(-50%)',background:'linear-gradient(135deg,#F43F5E,#E11D48)',color:'#fff',fontSize:11,fontWeight:700,padding:'4px 12px',borderRadius:100,whiteSpace:'nowrap' }}>{T.pricing.popular}</div>}
                  <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:6 }}>
                    <div style={{ width:10,height:10,borderRadius:'50%',background:i===1?'#FB7185':plan.color,boxShadow:`0 0 12px ${i===1?'#FB7185':plan.color}`,flexShrink:0 }} />
                  ...
                  <div style={{ marginBottom:16 }}>
                    {billing !== 'monthly' && (
                      <div style={{ fontSize: 13, color: 'rgba(240,244,255,.4)', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ textDecoration: 'line-through' }}>${(plan.price + 5).toFixed(2)}</span>
                        <span style={{ 
                          ...
                          {billing === 'threeYears' ? (
                            (plan.name === 'Agency' || plan.name === 'Agency Pro') ? (
                              `${T.billing.savings} $${Math.round(((plan.price + 5) * 36) - Number(getTotalPrice(plan.price + 5)))}`
                            ) : (
                              T.billing.save30
                            )
                          ) : (
                  ...
                  <div style={{ fontSize:12,color:'rgba(240,244,255,.5)',marginBottom:24,fontWeight:300, lineHeight: 1.6 }}>
                    <div>{T.billing.payToday} <span style={{fontWeight:600}}>${getTotalPrice(plan.price + 5)}</span> {T.billing.today}.</div>
                    <div style={{ marginTop: 2, fontSize: 11, opacity: 0.8 }}>
                      {T.billing.renewsAt} <span style={{fontWeight:600}}>${(plan.price + 5).toFixed(2)}{T.pricing.mo}</span>. {billing !== 'monthly' && `${T.billing.prepaidFor} ${billing === 'quarterly' ? 3 : billing === 'yearly' ? 12 : 36} ${T.billing.months}. `}{T.billing.exVat}
                    </div>
                  </div>
